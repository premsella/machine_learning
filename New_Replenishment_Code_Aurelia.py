...
inw_wh_name = 'MANDI'
date        = '10-Feb-2020'
prevdate   = '09-Feb-2020'
output      = '10Jan20'
#1 - Base Files
sm     = pd.read_csv('../Processed/BaseFiles/TCNS_Live_StoreMaster.csv')
#pm     = pd.read_parquet('../BaseFiles/Product_Master_Aurelia_(05-Sep-2019).parquet')
pm     = pd.read_parquet('../Processed/BaseFiles/Product_Master_'+brand+'_SS20.parquet')  ##################################################################
...
#2 - Input Files
pickuplist        = pd.read_excel('../InputFiles1/AURELIA core process status.xlsx', 'Item wise')
latest_inv        = pd.read_parquet('../InputFiles1/STOREINV_('+prevdate+').parquet')
wh_cur_inv        = pd.read_csv('../InputFiles1/CORE_WHINV_('+date+').csv')         #####################################################################
bsq_data          = pd.read_parquet('../InputFiles1/BSQ_Data.parquet')
stock_transfer_df = pd.read_parquet('../InputFiles1/Stock_Transfer_Movement.parquet')
ist_orders        = pd.read_csv('../InputFiles1/AURELIA_IST_'+date+'_All_Clusters.csv')
fofo_ist          = pd.read_csv('../InputFiles1/AURELIA_IST_COCO_FOFO_'+date+'.csv')
Sales             = pd.read_parquet('../InputFiles1/Sales_(01-Jan-2019 to '+prevdate+').parquet')
fofo_inv	  = pd.read_csv('../InputFiles1/FOFOINV_('+prevdate+').csv')
fofo_bsq	  = pd.read_parquet('../InputFiles1/AURELIA_FOFO_BSQ.parquet')
#Daily_inv 	  = pd.read_parquet('../InputFiles1/Daily_Inventory.parquet')

#3 Checking Valid Ouput Directory
pd.DataFrame(columns = ['Test']).to_parquet('../Output/'+output+'/'+brand+'/'+'Test.parquet')

#Theta Treatment for Aurelia
theta['ROS_THRES'] = theta['ROS_THRES']
...
sm1 = sm[(sm[brand] == 1)&(sm['Channel'] == 'OWN')&(sm.Rep_Status=='ACTIVE')].reset_index(drop=True) #**$**
sm2 = sm[(sm[brand] == 1)&(sm['Channel'] != 'OWN')&(sm.Rep_Status!='CLOSED')].reset_index(drop=True)
sm = pd.DataFrame()
...
#bsq_data.FAQ.fillna(3,inplace = True)
...
wh_cur_inv = wh_cur_inv.groupby(['Material'])['WH_Qty'].sum().reset_index()
...
pm['Priority_Product'].fillna(False, inplace=True)

#cc_sku = pm[pm['Priority_Product']==True]['Material'].unique()   #################################################################################
cc_sku = pm.Material.unique()
...
#     #now for partial fulfillable SKUs we will go by the priority list of channelsi
    for sku in cc_sku: #changed to cc_sku from part_sku
...
#Style_sales1 = Sales1.groupby(['Store_Code', 'Style_Code'])['Quantity'].sum().reset_index().rename(columns = {'Quantity':'Sales_3W'})
